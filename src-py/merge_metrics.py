...
def create_df(ver):
    ...
    metrics = metrics[['filename', 'TCchar', 'CountLineCode','N','NN','NF','SumCyclomatic','chum','relatedChum','delectChum','ncdChum', 'fault']]
    nml_metrics =  metrics[['filename', 'TCchar', 'CountLineCode','N','NN','NF','SumCyclomatic','fault']]
    ...
    # pd.DataFrame(metrics).to_csv( './../metrics/rfn/solr_ex1_rfn_' + ver +'.csv', index=False, cols=None)
    # pd.DataFrame(nml_metrics).to_csv( './../metrics/nml/solr_ex1_nml_' + ver +'.csv', index=False, cols=None)
    return metrics
