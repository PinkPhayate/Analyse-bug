...
def create_df(ver):
    ...
    # product metrics
    productM = pd.read_csv('./../metrics/prdctM/productMetricsByMyTool_v' + ver + '.csv', header=None)
    productM.columns = ['filename', 'TCchar', 'LOC2','N1','N2','eta1','eta2','N','NN','NF']
    productM = removePath(productM)
    # productM.rename(columns={0: 'filename'}, inplace=True)
    print productM.size
    #    'filename',NumOfBug,'LOC','chum','relatedChum','delectChum','ncdChum','isNewModule'

    #  import metcis by understand
    df = pd.read_csv('./../metrics/prdctM/solr' + ver + '.csv', header=0)
    ...
    # 'filename','AvgCyclomatic','CountLineCode','SumCyclomatic'
    ...
    # [u'filename', u'TCchar', u'LOC2', u'N1', u'N2', u'eta1', u'eta2', u'N', u'NN', u'NF', u'AvgCyclomatic', u'CountLineCode', u'SumCyclomatic']

    # process metrics
    processM = pd.read_csv('./../metrics/prcssM/processMetrics_v' + ver + '.csv', header=None)
    ...
    metrics = df.dropna()
    # [u'filename', u'TCchar', u'LOC2', u'N1', u'N2', u'eta1', u'eta2', u'N',u'NN', u'NF', u'AvgCyclomatic', u'CountLineCode', u'SumCyclomatic',u'NumOfBug', u'LOC', u'chum', u'relatedChum', u'delectChum', u'ncdChum',u'isNewModule'],
    metrics = metrics[['filename', 'TCchar', 'CountLineCode','N','NN','NF', 'chum','relatedChum','delectChum','ncdChum', 'isNewModule']]

    # create bug module list
    df = pd.read_csv('./../metrics/numBug/slr_'+ver+'_bgmd.csv', header=0)
    ...
    metrics.columns = ['filename', 'TCchar', 'CountLineCode','N','NN','NF', 'chum','relatedChum','delectChum','ncdChum', 'isNewModule', 'fault']
    metrics = metrics[['filename', 'TCchar', 'CountLineCode','N','NN','NF', 'chum','relatedChum','delectChum','ncdChum', 'fault']]
    ...
    pd.DataFrame(metrics).to_csv( './../result/record_ex1_nml_' + ver +'.csv', index=False, cols=None)
    return metrics
